...
from liteflownet.flowlib import flow_to_image
...
class Network(torch.nn.Module):
	def __init__(self):
		...
		print('load_state_dict(torch.loadnetwork-kitti.pytorch')
		self.load_state_dict(torch.load('liteflownet/network-' + 'kitti' + '.pytorch'))
		# self.load_state_dict(torch.load('network-' + 'kitti' + '.pytorch'))
	# end
...
# 					# nn.Upsample
# 	print(tensorPreprocessedFirst.size())
...
# ##########################################################
# def flownet(tensorFirst, tensorSecond):
# 	assert(tensorFirst.size(1) == tensorSecond.size(1))
# 	assert(tensorFirst.size(2) == tensorSecond.size(2))
...
# 	pred_flow = tensorOutput.numpy().transpose((1, 2, 0))
