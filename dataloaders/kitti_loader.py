...
def load_calib():
    ...
    V2C = [7.533745e-03 ,-9.999714e-01, -6.166020e-04 ,1.480249e-02, 7.280733e-04 ,-9.998902e-01, 9.998621e-01, 7.523790e-03, 1.480755e-02,-4.069766e-03 ,-7.631618e-02 ,-2.717806e-01]
    V2C = np.array(V2C)
    V2C = np.reshape(V2C, [3, 4])
    C2V = inverse_rigid_trans(V2C)
    # Rotation from reference camera coord to rect camera coord
    R0 = lines[8]
    R0 = R0.split(":")[1].split(" ")[1:]
    R0 = np.reshape(np.array([float(p) for p in R0]),(3,3)).astype(np.float32)
...
def train_transform(rgb, sparse, target, rgb_near, args,sparse2,sparse3,rgb1,rgb3):
...
def val_transform(rgb, sparse, target, rgb_near, args,sparse2,sparse3,rgb1,rgb3):
    transform = transforms.Compose([
        transforms.BottomCrop((oheight, owidth)),
    ...
    def __getraw__(self, index):
        ...
        if curr_file == later_file:  
            ...
            rgb_near = get_rgb_near(self.paths['rgb'][index+3], self.args) if \
                self.split == 'train' and self.args.use_pose else None
        return rgb, sparse, target, rgb_near,file,sparse2,sparse3,rgb1,rgb3

    def __getitem__(self, index):
        rgb, sparse, target, rgb_near,file ,sparse2,sparse3,rgb1,rgb3= self.__getraw__(index)
        rgb, sparse, target, rgb_near,sparse2,sparse3 ,rgb1,rgb3= self.transform(rgb,sparse, target, rgb_near,self.args, sparse2,sparse3,rgb1,rgb3)
        ...
        candidates = {"rgb":rgb, "d":sparse, "gt":target,\
                        "d2":sparse2, "d3":sparse3,"rgb1":rgb1,"rgb3":rgb3,}
